          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <Link href="/auth/forgot-password" className="font-body text-xs text-[#2041d8] hover:underline">
                  Forgot password?
                </Link>
              </div>
              <PasswordInput
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 text-red-600 text-sm font-body px-4 py-3 rounded-xl space-y-2">
                <p>{error}</p>
                {error === "Email not confirmed" && (
                  resent ? (
                    <p className="text-green-600">Confirmation email sent — check your inbox.</p>
                  ) : (
                    <button
                      type="button"
                      onClick={handleResendConfirmation}
                      disabled={resending}
                      className="text-[#2041d8] underline"
                    >
