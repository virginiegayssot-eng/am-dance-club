  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <div className="flex flex-col flex-1 overflow-hidden max-w-6xl mx-auto w-full px-0 sm:px-6 py-0 sm:py-6 md:flex-row gap-4">
        ...
        {me?.role !== "instructor" && (
          <div className="md:hidden flex flex-row gap-2 px-3 pt-3 shrink-0">
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl font-body text-sm text-center transition-colors inline-flex items-center justify-center gap-1.5 ${activeChannel === "group" ? "bg-[#334155] text-white" : "bg-white border border-gray-200 text-black"}`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2.5 rounded-xl font-body text-sm text-center transition-colors ${activeChannel !== "group" ? "bg-[#334155] text-white" : "bg-white border border-gray-200 text-black"}`}
            >
              Message Instructor
            </button>
            <a
        ...
        {me?.role === "instructor" && (
          <div className="md:hidden shrink-0 bg-white border-b border-gray-100">
            <div className="flex overflow-x-auto gap-2 px-3 pt-3 pb-2 scrollbar-hide">
              <button
                onClick={() => { setActiveChannel("group"); setDmTarget(null); }}
                className={`flex-shrink-0 px-4 py-2 rounded-xl font-body text-sm transition-colors inline-flex items-center gap-1.5 ${activeChannel === "group" ? "bg-[#334155] text-white" : "bg-gray-100 text-black"}`}
              >
              ...
              {students.map(s => {
                ...
                return (
                  <button
                    key={s.id}
                    onClick={() => selectDM(s)}
                    className={`flex-shrink-0 flex items-center gap-1.5 px-4 py-2 rounded-xl font-body text-sm transition-colors ${isActive ? "bg-[#334155] text-white" : "bg-gray-100 text-black"}`}
                  >
                    {s.full_name?.split(" ")[0] ?? s.email}
                    {unread > 0 && (
                      <span className={`text-xs font-heading w-4 h-4 rounded-full flex items-center justify-center ${isActive ? "bg-white text-[#334155]" : "bg-[#334155] text-white"}`}>
                        {unread}
                      </span>
                    )}
                  </button>
        ...
        {/* Sidebar */}
        <aside className="hidden md:flex flex-col w-64 shrink-0 bg-white rounded-2xl border border-[#e2e8f0]/30 overflow-hidden">
          {/* Group chat */}
          <button
            onClick={() => { setActiveChannel("group"); setDmTarget(null); }}
            className={`flex items-center gap-3 px-4 py-3.5 text-left border-b border-gray-50 transition-colors ${activeChannel === "group" ? "bg-[#334155] text-white" : "hover:bg-gray-50"}`}
          >
            <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${activeChannel === "group" ? "bg-white/20" : "bg-[#94a3b8]/30"}`}>
              <MessageCircle className={`w-4 h-4 ${activeChannel === "group" ? "text-white" : "text-[#334155]"}`} strokeWidth={1.75} />
            </div>
            <div>
            ...
            {me?.role === "instructor" ? (
                ...
                {students.map(s => {
                  ...
                  return (
                    <button
                      key={s.id}
                      onClick={() => selectDM(s)}
                      className={`w-full flex items-center gap-3 px-4 py-3 text-left transition-colors ${isActive ? "bg-[#e2e8f0]/30" : "hover:bg-gray-50"}`}
                    >
                      <div className="w-8 h-8 rounded-full bg-[#e2e8f0]/50 overflow-hidden flex items-center justify-center text-xs font-heading shrink-0">
                      ...
                      {unread > 0 && (
                        <span className="bg-[#334155] text-white text-xs font-heading w-5 h-5 rounded-full flex items-center justify-center shrink-0">
                          {unread}
                        </span>
                      )}
                    </button>
              ...
              <div className="px-4 py-3">
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors ${activeChannel !== "group" ? "bg-[#e2e8f0]/30" : "hover:bg-gray-50"}`}
                >
                  <div className="w-8 h-8 rounded-full bg-[#334155]/10 flex items-center justify-center">
                    <GraduationCap className="w-4 h-4 text-[#334155]" strokeWidth={1.75} />
                  </div>
          ...
          >
            <MessageCircle className="w-5 h-5 text-[#334155] shrink-0" strokeWidth={1.75} />
            <div className="flex-1 min-w-0">
        ...
        {/* Chat panel */}
        <div className="flex-1 flex flex-col bg-white sm:rounded-2xl border border-[#e2e8f0]/30 overflow-hidden">
          {/* Header */}
          <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-[#f8fafc]">
            <div className="w-9 h-9 rounded-full bg-[#e2e8f0]/50 flex items-center justify-center text-sm font-heading">
              {activeChannel === "group" ? <MessageCircle className="w-4 h-4 text-[#334155]" strokeWidth={1.75} /> : (dmTarget?.full_name ?? "?")[0]?.toUpperCase()}
            </div>
            <div>
          ...
          <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full gap-2">
                <MessageCircle className="w-9 h-9 text-[#334155]" strokeWidth={1.5} />
                    ...
                    <div className={`max-w-[75%] ${isMe ? "items-end" : "items-start"} flex flex-col gap-1`}>
                      ...
                      {isEditing ? (
                        <div className="flex gap-2 items-center">
                          ...
                          />
                          <button onClick={() => saveEdit(msg.id)} className="font-body text-xs text-[#334155] hover:underline">Save</button>
                        ...
                        <div className="flex items-end gap-1.5">
                          ...
                          <div className={`rounded-2xl overflow-hidden font-body text-sm leading-relaxed ${
                            isMe
                              ? "bg-[#334155] text-white rounded-br-sm"
                              : "bg-[#e2e8f0]/25 text-black rounded-bl-sm"
                          }`}>
                            {msg.image_url && (
          ...
          <div className="border-t border-gray-100 bg-[#f8fafc]">
            ...
            <form onSubmit={sendMessage} className="flex items-center gap-3 px-4 py-3">
              ...
              <button
                ...
                disabled={sending}
                className="text-gray-400 hover:text-[#334155] shrink-0"
