...
function SectionHeader({ icon: Icon, title, description }: { icon: LucideIcon; title: string; description: string }) {
  return (
    <div className="flex items-start gap-3 mb-6">
      <div className="w-10 h-10 rounded-xl bg-[#221f1c]/10 flex items-center justify-center shrink-0">
        <Icon className="w-5 h-5 text-[#221f1c]" strokeWidth={1.75} />
      </div>
      <div>
                    ...
                    <div
                      ...
                      className={`card p-5 transition-all hover:-translate-y-0.5 hover:shadow-md ${
                        isToday
                          ? "bg-[#221f1c]"
                          : isThisWeek
                          ? "bg-white ring-1 ring-[#221f1c]/25"
                          : ""
                      }`}
                    >
                      <div className="flex items-start justify-between mb-3 gap-3">
                        ...
                        <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                          isToday ? "bg-white/15" : isThisWeek ? "bg-[#221f1c]/10" : ""
                        }`}>
                          {isToday ? (
                            <PartyPopper className="w-5 h-5 text-white" strokeWidth={1.75} />
                          ) : isThisWeek ? (
                            <Cake className="w-5 h-5 text-[#221f1c]" strokeWidth={1.75} />
                          ) : (
                      ...
                      <div className="mt-2">
                        {isToday ? (
                          <span className="inline-flex items-center bg-white text-[#221f1c] font-heading text-xs px-2.5 py-1 rounded-full">
                            🎂 Today!
                          </span>
                        ) : isThisWeek ? (
                          <span className="inline-flex items-center bg-[#221f1c] text-white font-heading text-xs px-2.5 py-1 rounded-full">
                      ...
                      <Link
                        href={`/chat?dm=${s.id}`}
                        className={`mt-3 font-body text-xs hover:underline block ${isToday ? "text-white" : "text-[#221f1c]"}`}
                      >
            ...
            <div className="card p-6 sm:p-8 space-y-6">
                    ...
                    return (
                      <button
                        ...
                        className={`font-body text-sm px-4 py-2 rounded-full whitespace-nowrap transition-colors ${
                          active ? "bg-[#221f1c] text-white" : "bg-white text-gray-500 border border-gray-200 hover:text-black"
                        }`}
                      >
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={selectAllInSegment} className="font-body text-xs text-[#221f1c] hover:underline">Select all</button>
                </div>
                  ...
                  <div className="space-y-1 max-h-44 overflow-y-auto bg-white border border-gray-100 rounded-xl p-2">
                    {currentSegment.map(s => {
                      ...
                      return (
                        <label
                          key={s.id}
                          className={`flex items-center gap-3 font-body text-sm px-2 py-1.5 rounded-lg cursor-pointer transition-colors ${checked ? "bg-[#221f1c]/5" : "hover:bg-gray-50"}`}
                        >
                          <input type="checkbox" className="shrink-0" checked={checked} onChange={() => toggleBroadcastRecipient(s.id)} />
                          <span className="w-7 h-7 rounded-full bg-[#221f1c]/10 text-[#221f1c] font-heading text-[11px] flex items-center justify-center shrink-0">
