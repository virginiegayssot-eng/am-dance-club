      ...
      <main className="flex-1 max-w-2xl mx-auto px-4 sm:px-6 py-14 w-full">
        ...
        <div className="card p-8">
          ...
          <div className="flex flex-col items-center mb-8">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-[#e4c3cc]/50 mb-3">
              {avatarUrl ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={avatarUrl} alt="Profile photo" className="w-full h-full object-cover" />
              ) : (
                <div className="w-full h-full flex items-center justify-center font-heading text-3xl text-[#2041d8]">
                  {(form.full_name || profile?.email || "?")[0].toUpperCase()}
                </div>
              )}
            </div>
            <label className="cursor-pointer font-body text-sm text-[#2041d8] hover:underline">
            ...
            {profile?.role === "instructor" && (
              <>
                <div>
                  ...
                  <input type="text" className="input" placeholder="e.g. Founder" value={form.title} onChange={set("title")} />
                  <p className="font-body text-xs text-gray-400 mt-1">Shown under your name on the public Instructors page.</p>
                </div>
                <div>
                  <label className="label">Bio</label>
                  <textarea className="input min-h-[120px]" placeholder="Tell people about yourself…" value={form.bio} onChange={set("bio")} />
                </div>
              </>
            )}
          ...
          <div className="card p-8 mt-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <button
                ...
                onClick={togglePush}
                disabled={pushBusy}
                className={`shrink-0 w-12 h-7 rounded-full transition-colors relative ${pushEnabled ? "bg-[#2041d8]" : "bg-gray-200"} ${pushBusy ? "opacity-60" : ""}`}
              >
