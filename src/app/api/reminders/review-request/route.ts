...
// Called on a schedule (every 15 min) by pg_cron — see
// supabase/add-review-request-reminders.sql. Emails first-time attendees a
// review request ~1 hour after their class ends, once each, tracked via
// attendance.review_reminder_sent_at. Runs against every class's real
// start/end time rather than a single fixed daily time, since BYLA (unlike
// THE A.M's one-class-a-week schedule) runs classes on multiple different
// days at different times. "First time" mirrors the manual
// review-candidates check: no other attended=true row for that student.
export async function POST(req: NextRequest) {
  ...
  // A coarse date-range prefilter — the precise "ended ~1 hour ago" check
  // happens below in JS, in real Sydney time via sydneyDateTimeToUTC.
  ...
  const due = (candidates as any[]).filter((row) => {
    ...
    // ~1 hour after class ends, sized to how often this job runs (every 15 min).
  ...
  for (const row of due) {
    // Mark attempted immediately so a slow send (or a crash mid-loop) can't
    // cause a duplicate on the next tick.
    ...
    const { data: prevAttendance } = await admin
      ...
      .limit(1);
    if (prevAttendance && prevAttendance.length > 0) continue; // not a first-timer
    ...
    const { error } = await resend.emails.send({
      ...
      to: profile.email,
      subject: "Thanks for dancing with us tonight!",
