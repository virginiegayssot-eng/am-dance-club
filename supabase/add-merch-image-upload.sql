...
-- library instead of typing an image URL. Creates a public storage bucket
-- that /api/instructor/upload-merch-image writes to with the service role
-- key (bypasses RLS on upload), plus a public-read policy so product
-- photos actually render on the public /merch page.
