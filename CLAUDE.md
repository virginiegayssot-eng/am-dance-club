...
- [x] Automatic first-timer review-request emails — built 3 Sep, see "Automatic reminders feature" above for why this uses a generic "class end + 1h" cron (`/api/reminders/review-request`, every 15 min) rather than a fixed daily time, since BYLA runs classes on multiple different days/times. Toggle lives in Marketing > Reminders (new admin-only tab, single switch — not the four-switch layout THE A.M has, since booking/win-back/birthday aren't built here). Reuses the existing in-app `buildReviewEmailHtml` template as-is. Migration: `supabase/add-review-request-reminders.sql` (not yet run against BYLA's Supabase project — needs the real app URL and BYLA's own `CRON_SECRET` filled in before running).
...
- [ ] Automatic booking reminders, win-back emails, and birthday emails — still not built here (only review-request is, see above).
...
- [x] Dashboard + Marketing pill-tab separator invisible against `#f4efe6` cream body (`border-gray-100`, same bug as Manea's) — fixed to `border-[#221f1c]/20` on both tab bars, same day the Marketing page shipped. See "Tab bar & panel styling" above — this exact bug still needs checking on BYLA.
