...
- [ ] **Bug found, not yet fixed:** the Dashboard pill-tab row's separator (`border-b border-gray-100` in `instructor/page.tsx`) is invisible against BYLA's actual `#e2d0fb` lavender body — same root cause as the Manea/`demo` instances of this bug documented in "Tab bar & panel styling" above. Fix to `border-[#000000]/20` (already used by BYLA's own `Navbar.tsx`) whenever next touching this file — bundle it with the Marketing build below since that page will need the same fix on its own copy of the tab bar.
- [x] Stale "Filming policy accepted" Members badge fixed to "Terms & Conditions accepted" — 29 Aug.
- [ ] Marketing section (`/marketing` page + Navbar link) — not built. When it is, build it with the pill-tab/`SectionHeader`/card-panel styling from the start (see "Tab bar & panel styling" above, including the separator-color check), not the older plain-underline look THE A.M briefly shipped with.
