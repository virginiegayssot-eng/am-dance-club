...
## Guest booking (BYLA, 17 Sep)

A member can check "Bring a guest (+1 spot)" on `/classes` before booking, covering the two paths the user actually described (BYLA has no "Double Pass" product like THE A.M does — that pass type was removed a while back, so this is a different mechanism, not a revival of it):

- **Pass holder** — `/api/bookings` already computed `creditsNeeded = 1 + Math.max(0, guestCount - maxGuests)` (a pass type can carry a `max_guests` allowance that covers a guest for free, e.g. a hypothetical "Double Pass"-equivalent) and deducted/stored it correctly. The backend was fully built; the frontend just always sent `guestCount: 0` and had no checkbox. That was the whole gap on this side.
- **Casual (no pass)** — genuinely new: `/api/stripe/pass-checkout` now accepts `guestCount`, sets the Stripe line item `quantity: 1 + guestCount` (charges per person, not per booking), and passes `guestCount` through session metadata; the webhook reads it from there to set `registrations.guest_count` on the confirmed booking.
- **Cleaned up in the same pass:** the webhook had dead code from BYLA's old "Double Pass" era (`isDoublePass` metadata, `passTypeId === "double"` branches) that could never actually run since `PASS_CONFIGS` no longer has a `"double"` key — removed rather than left as confusing dead branches next to the new real `guestCount` logic.
- **Capacity bug found and fixed along the way:** `class_registration_counts` counted registration rows (`count(*)`), not real attendees — a member bringing a guest only ever counted as 1 toward capacity even though 2 people show up. Changed to `sum(1 + guest_count)`. Needs `supabase/add-guest-capacity-fix.sql` run once against BYLA's real Supabase project (idempotent `create or replace view`).
- **Not fixed, flagged for later:** the casual/Stripe booking path has never enforced class capacity at all (no capacity check anywhere in `pass-checkout/route.ts`) — a pre-existing gap, unrelated to guests, out of scope for this change.
- Not yet ported anywhere else — this was built specifically for BYLA's reported case. THE A.M has its own real "Double Pass" product instead (a different mechanism, not this one), so check what a client actually wants before assuming this is the right shape to copy elsewhere.

## Edit Class (Dashboard)

Started on BYLA — before this, THE A.M/Manea/`demo`'s "New Class" modal only ever inserted; once a class existed, the only options were Cancel or Delete, so fixing a typo'd price or a wrong time meant deleting and recreating (losing bookings). BYLA's Dashboard adds an "Edit" button to each `ClassRow` that reopens the same modal pre-filled (`openEditClass(cls)`), with a single `saveClass()` that inserts or updates depending on whether an `editingClassId` is set (`openNewClass()` clears it back to create-mode). Ported to Manea 29 Aug (its category/alt-duration/special-class fields carried straight through into the edit form too, since it already had them on create — no need to strip anything down to match BYLA's leaner field set), then to THE A.M and `demo` on 10 Sep in the same session (the `demo` port also added try/catch/finally to `saveClass()`, which THE A.M's version already had but `demo`'s pre-existing `createClass()` didn't — see the async-form-handler rule above). All four clients now have it.
...
## Classes page: show the class title, and description as an expandable toggle

The public `/classes` page card never rendered `classes.title` or `classes.description` at all on THE A.M, `demo`, or BYLA — the data was always saved correctly (the New Class/Edit Class form has always had both fields), just never displayed. Manea already had this right (`ReviewRequestPanel`'s neighbour on the classes card, not a component — just inline JSX): the card header leads with the title (`font-heading text-2xl`, day/date as a smaller line underneath, not the other way around), and if `description` is set it becomes a "Class details ↓" button that expands a paragraph on click, rather than always showing. Fixed on THE A.M and `demo` (10 Sep) to match Manea's exact pattern — shown for *any* class with a description, not gated to `is_special`, since a regular class occasionally having one (e.g. a one-off note) shouldn't be hidden either. **BYLA still has this gap** — not yet fixed there. Needs its own `expandedDescId` state distinct from whatever state already drives that page's "More options" payment-method toggle (`expandedId` on THE A.M/`demo`) — don't reuse the same state for both, they're independent UI elements that can both be open on the same card at once.

## Automatic reminders feature

Built first on THE A.M (`main`), then on `template`: booking reminders (email + push, ~12h before a confirmed class, to registered students only), win-back emails (members inactive 3+ weeks), birthday emails (added 27 Aug, `main` only so far — sent same-day, matched on `birth_date`'s month/day in Sydney time since the year on that column is just a signup placeholder), and automatic first-timer review-request emails (added 27 Aug on `main`, then 3 Sep on BYLA — see the timing note below for why BYLA's version fires on class-end-relative timing rather than a fixed daily time; asks a student for a review after their *first ever* attended class — "first-timer" for a given `attendance` row means the student has no other `attended = true` row, mirroring the existing manual review-candidates check exactly, just run on a schedule instead of on-demand). All four run behind a Supabase `pg_cron` schedule hitting `/api/reminders/booking`, `/api/reminders/winback`, `/api/reminders/birthday`, and `/api/reminders/review-request`, each gated by a `CRON_SECRET` header check, and a `reminder_settings` singleton row toggleable per-type from Marketing > Reminders (admin-only, so needs the `is_admin` column above first — see "Marketing section" below, this used to live on Reports > Settings but moved). All four now exist on THE A.M, BYLA, and Manea (see each client's checklist below) — `demo` deliberately has none (see its checklist: the settings/UI aren't built there since there's no `is_admin` tier to gate Reminders behind, and it isn't a live client anyway), and `template` still only has booking + win-back. When porting:
...
- **Review-request timing is same-day and per-client, unlike the other three — and the right architecture depends on whether the client has one weekly class or several.** THE A.M runs a single Friday class, so a fixed daily cron time works: Friday 8am Sydney (`0 22 * * 4` UTC — Thursday 22:00 UTC, since Friday 8am Sydney falls on the UTC-Thursday side), with email copy naming the time of day ("this morning"). **A client with classes on multiple different days/times (BYLA, and Manea will be the same) can't use one fixed cron time at all** — a single daily time is either wrong for most classes or requires a separate cron entry per class-day, which breaks the moment the schedule changes. Built for BYLA instead as a generic "class end + 1 hour" job: `/api/reminders/review-request` runs every 15 minutes via `pg_cron`, computes each class's real end time in Sydney time (`sydneyDateTimeToUTC` + `duration_minutes`, added to `src/lib/date.ts`), and fires for any first-time attendee whose class ended 45–75 minutes ago — deduped via `attendance.review_reminder_sent_at` so the 15-minute polling can't double-send. This is the same shape as THE A.M's existing booking-reminder job (`/api/reminders/booking`, ~12h-before check every 15 min), just applied to "after" instead of "before." Reuse this generic pattern for Manea's build too rather than trying to pick one fixed daily time for a multi-day schedule — and since the email itself never states a fixed time of day ("thanks for dancing with us tonight" reads fine whenever it fires), it doesn't need the same-day copy rewrite THE A.M's fixed-time version does.
...
- **Win-back cadence bug (found 17 Sep, fixed on `main`/`byla`/`demo`/`template`):** the original win-back route re-fired the "we miss you" email every single 21 days, forever, for anyone who stayed inactive — no cap. Fixed to an escalating 3-stage schedule instead: 21 days, then 60, then 120, then nothing further until they attend again (which resets the sequence). Needs a `winback_count` int column on `profiles` (`supabase/add-winback-escalation.sql`, run once per client's real Supabase project) alongside the existing `last_winback_sent_at`. Manea doesn't have reminders built at all yet, so nothing to port there.
...
- **`border-gray-100` isn't a universal separator color — check the client's actual `<body>` background before reusing it.** It reads fine on THE A.M (`#e4c3cc` pink), but every other client's body is close enough in lightness to `gray-100` that the pill row's bottom border goes effectively invisible while the class sits right there in the code, unnoticed: Manea (`#FCF4E9` pale cream), BYLA (`#e2d0fb` light lavender), and `demo`/Sable Studio (`#f4efe6` pale cream) all had this exact bug — found and fixed on all three now (Manea and `demo` first, then BYLA's Dashboard *and* its new Marketing page's copy of the tab bar in the same pass that built Marketing there). This was a recurring miss, not a one-off — it was fixed once on Manea, then shipped broken again on `demo` in the same session. **Do not trust the "reads fine on X" note from a prior pass; re-verify against the client's real `globals.css` `body {}` rule every single time**, including on branches already believed fixed. The fix is always the same and already sitting in that client's own `Navbar.tsx`, which never used `gray-100` in the first place: Manea → `border-primary/20`, BYLA/THE A.M → `border-[#000000]/20` / `border-[#2041d8]/20` (check the actual file, don't assume), `demo` → `border-[#221f1c]/20`. Whenever porting this tab bar (or a Marketing page's copy of it) to a new client — Manea's still needed on `template`'s eventual build — grep that branch's own `Navbar.tsx` for its border color and reuse it, don't copy `border-gray-100` on faith even if a past session's notes said it was fine.
...
- [x] Dashboard + Marketing pill-tab separator invisible against BYLA's `#e2d0fb` lavender body (`border-gray-100`, same bug as Manea/`demo`) — fixed to `border-[#000000]/20` on both tab bars, same day the Marketing page shipped.
- [x] Stale "Filming policy accepted" Members badge fixed to "Terms & Conditions accepted" — 29 Aug.
- [x] Marketing section (`/marketing` page + Navbar link) — built, with the pill-tab/`SectionHeader`/card-panel styling from the start. Review Requests and Message a Segment (new `/api/instructor/broadcast-email`, BYLA's own voice/sign-off) open to every instructor, same as the old ungated header button; Reviews carousel, Merch (still behind `MERCH_ENABLED`, currently off), Discounts, Birthdays (moved off Reports, fixing the same-day date-math bug along the way), and Reminders (see below) kept admin-only, preserving the access level they already had as Dashboard/Reports tabs rather than loosening it.
- [x] Merch/Discounts/Reviews-carousel moved off the Dashboard into Marketing — done, extracted into `MerchPanel.tsx`/`DiscountsPanel.tsx`/`ReviewsCarouselPanel.tsx` (new shared `src/components/Modal.tsx`; `instructor/page.tsx` keeps its own local `Modal` for its remaining forms). `DiscountsPanel` keeps BYLA's own "Applies to" pass-type filter (`applicable_pass_type`); `MerchPanel` keeps BYLA's plain image-URL field, not the photo-library upload — that's the separate item below.
- [x] Automatic first-timer review-request emails — built 3 Sep, see "Automatic reminders feature" above for why this uses a generic "class end + 1h" cron (`/api/reminders/review-request`, every 15 min) rather than a fixed daily time, since BYLA runs classes on multiple different days/times. Reuses the existing in-app `buildReviewEmailHtml` template as-is. Migration run against BYLA's real Supabase project.
- [x] Automatic booking reminders, win-back emails, and birthday emails — built and migration run (`supabase/add-booking-winback-birthday-reminders.sql`), reusing the same `reminder_settings` table/CRON_SECRET as review-request. Marketing > Reminders now shows all four toggles, matching THE A.M's layout. Booking reminders also push via the existing `push_subscriptions` infra.
...
- [ ] Birthday card redesign in Marketing > Birthdays — the tab exists now (see above); a visual redesign of the cards themselves is still open.
- [ ] Merch image upload (photo library instead of URL) — not ported, see "Merch image upload" above.
- [ ] Classes page doesn't show the class title or an expandable description toggle yet, unlike THE A.M/`demo`/Manea — see "Classes page: show the class title, and description as an expandable toggle" above.
...
- [x] Edit Class capability added, matching BYLA — 29 Aug, see "Edit Class (Dashboard)" above. THE A.M and `demo` got it too on 10 Sep, so all four clients now have it.
- [x] Automatic reminders — **this checklist previously said "none of the four exist here yet," which was wrong.** All four (`booking`/`winback`/`birthday`/`review-request` routes, the `reminder_settings` table, the Marketing > Reminders toggle UI) are fully built and already in Manea's own voice, signed "Manea." What's actually still outstanding: `supabase/add-reminders.sql` has never been run against Manea's real Supabase project — until it is, the Reminders tab just shows "Reminders aren't set up on this project yet." Also fixed the same win-back-repeats-every-21-days-forever bug here as on main/byla/demo/template (17 Sep) — needs `supabase/add-winback-escalation.sql` run right after `add-reminders.sql`, in that order. Once both are run, the four `cron.schedule` blocks at the bottom of `add-reminders.sql` still need editing with Manea's real app URL and her own `CRON_SECRET`.
- [x] Review-request email already uses a Google review link in code (`process.env.GOOGLE_REVIEW_URL`, same pattern as THE A.M) — just needs the real `GOOGLE_REVIEW_URL` value set in Manea's Netlify env vars once she's live.
...
- [x] Stale "Filming policy accepted" Members badge fixed to "Terms & Conditions accepted" — 29 Aug.
- [x] Marketing section (`/marketing` page + Navbar link) — built, Merch/Discounts/Reviews-carousel moved off the Dashboard, Birthdays moved off Reports (fixing the same-day date-math bug along the way), plus a new Message a Segment broadcast tool (`/api/instructor/broadcast-email`, Sable Studio's own voice/branding). No `is_admin` column here, so none of the seven tabs are admin-gated (unlike THE A.M) — including Reminders, open to every instructor same as the rest.
- [x] Automatic reminders — all four built (Sable Studio's own voice/branding), with a Reminders tab and the `reminder_settings` toggles fully working, but the `cron.schedule` calls in the migration are deliberately left commented out since `demo` isn't a live client — a prospect can see the feature's settings without it actually sending real scheduled emails. Uncomment + fill in a real CRON_SECRET only if that's ever deliberately wanted here.
- [x] Dashboard + Marketing pill-tab separator invisible against `#f4efe6` cream body (`border-gray-100`, same bug as Manea's) — fixed to `border-[#221f1c]/20` on both tab bars, same day the Marketing page shipped.
- [x] Classes page shows the class title + an expandable "Class details ↓" description toggle, matching Manea — 10 Sep, see "Classes page: show the class title, and description as an expandable toggle" above.
- [x] Edit Class capability — 10 Sep, see "Edit Class (Dashboard)" above.
