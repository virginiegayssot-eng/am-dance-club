...
- [x] Marketing section (`/marketing` page + Navbar link) — built, with the pill-tab/`SectionHeader`/card-panel styling from the start. Six tabs: Review Requests and Message a Segment (new `/api/instructor/broadcast-email`, BYLA's own voice/sign-off) open to every instructor, same as the old ungated header button; Reviews carousel, Merch (still behind `MERCH_ENABLED`, currently off), Discounts, and Birthdays (moved off Reports, fixing the same-day date-math bug along the way) kept admin-only, preserving the access level they already had as Dashboard/Reports tabs rather than loosening it.
...
- [ ] Email copy for all four reminder templates — needs BYLA's own voice/sign-off/Instagram handle, not a copy-paste from THE A.M. (Message a Segment's one-off broadcast email above already has BYLA's voice — same voice, but that's a manual tool, not one of the four automatic reminders.)
